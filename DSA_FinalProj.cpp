...
int generateID(){
 ...
 void addResident(){
    ...
    printf("Resident Added Succesfully!");
...
void checkRole(){
	...
	char captainUser[20] = "Admin.123", input[10];
	char secretary[20] = "SECRETARY";
	
	//while(input!=captainUser || input!=secretary){
	printf("Enter Password: ");
	scanf("%s", input);	
	//}
	
	char role[10];
	if(!input==captainUser){
		role == "Secretary";
	}
	role=="Captain";
...
void displayResidents(){
	...
	while(current!=NULL){
		printf("ID: %d-4d \t Name: %s \t Age: %d \t Gender: %s", current->id, current->name, current->age, current->gender);
		current = current->next;
	}
}
void editResident(){
